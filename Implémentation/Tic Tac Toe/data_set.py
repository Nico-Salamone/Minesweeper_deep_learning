...
def generate_data_set(grid = ttt.initialize_empty_grid()):
	...
	other_player = ttt.Player.get_other_player(player)

	if state == ttt.State.WIN:
		return [(grid, (1, 0, 0), other_player)]
	elif state == ttt.State.LOSE:
		return [(grid, (0, 1, 0), other_player)]
	elif state == ttt.State.DRAW:
		return [(grid, (0, 0, 1), other_player)]
		...
		tree.insert(0, (grid, scores, other_player))
...
def read_data_set(data_set_file_name):
	...
	with open(data_set_file_name, newline = '') as data_set_file:
		...
		for row in csv_reader:
			...
			player = ttt.Player(int(row[n + 3]))

			data_set.append((grid, scores, player))
